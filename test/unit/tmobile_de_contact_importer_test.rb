...
class TmobileDeContactImporterTest < ContactImporterTestCase
  def setup
    super
    @account = TestAccounts[:tmobile_de]
  end
  ...
  def test_importer_fails_with_blank_password
    assert_raise(Contacts::AuthenticationError) do
      Contacts.new(:tmobile_de, @account.username, "")
    end
  end

  def test_importer_fails_with_blank_username
    assert_raise(Contacts::AuthenticationError) do
      Contacts.new(:tmobile_de, "", @account.password)
    end
  end

  def test_fetch_contacts
    contacts = Contacts.new(:tmobile_de, @account.username, @account.password).contacts
